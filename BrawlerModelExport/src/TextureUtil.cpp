	...
	namespace Texture
	{
		void WriteTextureToFile(const TextureWriteInfo& writeInfo)
		{
			...
			if (errorCode) [[unlikely]]
				throw std::runtime_error{ std::string{ "ERROR: Util::Texture::WriteTextureToFile() failed to create a texture's parent directory with the following error message: "} + errorCode.message() };
