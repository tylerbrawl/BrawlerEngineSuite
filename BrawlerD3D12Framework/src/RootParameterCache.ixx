...
export namespace Brawler
{
	namespace D3D12
	{
		...
		private:
			std::optional<Brawler::RootSignatures::RootSignatureID> mRootSignatureID;
...
namespace Brawler
{
	namespace D3D12
	{
		...
		void RootParameterCache::SetRootSignature()
		{
			...
			if (mRootSignatureID.has_value() && *mRootSignatureID == RSIdentifier)
				return;

			mRootSignatureID = RSIdentifier;

			mRootParamArr.clear();
			mRootParamArr.resize(std::to_underlying(Brawler::RootSignatures::RootParamEnumType<RSIdentifier>::COUNT_OR_ERROR));
		}
