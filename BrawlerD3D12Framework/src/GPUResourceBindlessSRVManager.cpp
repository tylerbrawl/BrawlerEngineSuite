...
namespace Brawler
{
	namespace D3D12
	{
		...
		{
			assert(!HasBindlessSRVs() && "ERROR: A GPUResourceBindlessSRVManager was destroyed before all of the BindlessSRVAllocation instances which it created were destroyed!");
		}
		...
		bool GPUResourceBindlessSRVManager::HasBindlessSRVs() const
		{
			return !mBindlessSentinelArr.Empty();
		}
