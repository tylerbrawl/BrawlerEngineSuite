...
namespace Brawler
{
	...
	{
		// These functions are defined in the .ixx files generated by the Brawler Shader Compiler.

		template <auto RSIdentifier>
		extern std::span<const std::uint8_t> GetSerializedRootSignature1_0();

		template <auto RSIdentifier>
		extern std::span<const std::uint8_t> GetSerializedRootSignature1_1();
	}
}

namespace
{
	template <auto RSIdentifier, D3D_ROOT_SIGNATURE_VERSION HighestVersion>
	__forceinline void AddRootSignatureCreationJob(std::array<Microsoft::WRL::ComPtr<Brawler::D3D12RootSignature>, std::to_underlying(decltype(RSIdentifier)::COUNT_OR_ERROR)>& rsArray, Brawler::JobGroup& jobGroup)
	{
		...
		jobGroup.AddJob([&rootSigObject] ()
		{
			...
			{
				auto rootSignatureSpan{ Brawler::RootSignatures::GetSerializedRootSignature1_1<RSIdentifier>() };
			...
			{
				auto rootSignatureSpan{ Brawler::RootSignatures::GetSerializedRootSignature1_0<RSIdentifier>() };
				CheckHRESULT(Util::Engine::GetD3D12Device().CreateRootSignature(
					0,
					rootSignatureSpan.data(),
					rootSignatureSpan.size_bytes(),
		...
		if constexpr ((std::to_underlying(RSIdentifier) + 1) != std::to_underlying(decltype(RSIdentifier)::COUNT_OR_ERROR))
			AddRootSignatureCreationJob<static_cast<decltype(RSIdentifier)>(std::to_underlying(RSIdentifier) + 1), HighestVersion>(rsArray, jobGroup);
	}
}
