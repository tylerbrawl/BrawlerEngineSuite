...
export module Brawler.ComputeRootParameterBinder;
...
namespace Brawler
{
	namespace IMPL
	{
		...
		{
			template <typename RootParamEnum, RootParamEnum RootParamID>
...
export namespace Brawler
{
	...
	class ComputeRootParameterBinder
	{
	private:
	...
	public:
		explicit ComputeRootParameterBinder(Brawler::D3D12GraphicsCommandList& cmdList);

		ComputeRootParameterBinder(const ComputeRootParameterBinder& rhs) = delete;
		ComputeRootParameterBinder& operator=(const ComputeRootParameterBinder& rhs) = delete;

		ComputeRootParameterBinder(ComputeRootParameterBinder&& rhs) noexcept = default;
		ComputeRootParameterBinder& operator=(ComputeRootParameterBinder&& rhs) noexcept = default;
...
namespace Brawler
{
	namespace IMPL
	{
		...
		{
			constexpr std::uint32_t ROOT_PARAM_INDEX = GetRootParameterIndex<RootParamEnum, RootParamID>();
			cmdList.SetComputeRoot32BitConstant(ROOT_PARAM_INDEX, resource, 0);
		}
		...
		{
			constexpr std::uint32_t ROOT_PARAM_INDEX = GetRootParameterIndex<RootParamEnum, RootParamID>();
			cmdList.SetComputeRootConstantBufferView(ROOT_PARAM_INDEX, resource->GetD3D12Resource().GetGPUVirtualAddress());
		}
		...
		{
			constexpr std::uint32_t ROOT_PARAM_INDEX = GetRootParameterIndex<RootParamEnum, RootParamID>();
			cmdList.SetComputeRootShaderResourceView(ROOT_PARAM_INDEX, resource->GetD3D12Resource().GetGPUVirtualAddress());
		}
		...
		{
			constexpr std::uint32_t ROOT_PARAM_INDEX = GetRootParameterIndex<RootParamEnum, RootParamID>();
			cmdList.SetComputeRootUnorderedAccessView(ROOT_PARAM_INDEX, resource->GetD3D12Resource().GetGPUVirtualAddress());
		}
		...
		__forceinline void ResourceBinder<RootParamType::DESCRIPTOR_TABLE>::BindResource(Brawler::D3D12GraphicsCommandList& cmdList, RootParameterBindingType<RootParamType, RootParamType::DESCRIPTOR_TABLE> resource)
		{
			...
			// Ensure that the relevant descriptors are copied into the ResourceDescriptorHeap.
			...
			cmdList.SetComputeRootDescriptorTable(ROOT_PARAM_INDEX, descriptorTableHandles.GPUHandle);
		}
	}

	template <Brawler::PSOID PipelineStateObjectID>
	ComputeRootParameterBinder<PipelineStateObjectID>::ComputeRootParameterBinder(Brawler::D3D12GraphicsCommandList& cmdList) :
		mCmdList(&cmdList)
	{}

	template <Brawler::PSOID PipelineStateObjectID>
	template <ComputeRootParameterBinder<PipelineStateObjectID>::RootParamEnumType RootParamID>
	void ComputeRootParameterBinder<PipelineStateObjectID>::BindResource(ResourceBindingType<RootParamID> resource)
	{
