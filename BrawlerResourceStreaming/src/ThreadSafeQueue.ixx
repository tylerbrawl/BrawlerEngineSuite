...
export namespace Brawler
{
	...
	public:
		...
		/// </summary>
		/// <returns>
		/// This function returns the size of the queue at the time of calling it. Keep in mind that due to
		/// race conditions, it is entirely possible that the size of the queue changes immediately after
...
namespace Brawler
{
	...
	template <typename T, std::size_t NumElements>
	std::size_t ThreadSafeQueue<T, NumElements>::GetSize() const
	{
		const QueueInfo queueInfo{ UnpackOffsets(mPackedOffsets.load()) };

		if (queueInfo.EndIndex >= queueInfo.BeginIndex)
			return (queueInfo.EndIndex - queueInfo.BeginIndex);

		// If queueInfo.BeginIndex > queueInfo.EndIndex, then
		// (queueInfo.BeginIndex - queueInfo.EndIndex) actually gives us the
		// number of *unused* slots in the queue. Seriously! Try it out!
		return (NumElements - (queueInfo.BeginIndex - queueInfo.EndIndex));
	}
